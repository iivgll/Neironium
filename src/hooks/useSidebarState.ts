...
interface SidebarState {
  ...
  chatToMoveToNewProject: number | null;
  editingChatId: number | null;
  editingChatTitle: string;
}

type SidebarAction =
  | { type: "SET_HOVERED_CHAT"; payload: number | null }
  | { type: "SET_HOVERED_PROJECT"; payload: number | null }
  ...
  | { type: "SET_CHAT_TO_MOVE_TO_NEW_PROJECT"; payload: number | null }
  | { type: "SET_EDITING_CHAT"; payload: { id: number | null; title: string } }
  | { type: "RESET_STATE" };
...
export function useSidebarState() {
  ...
  const setHoveredChatId = useCallback((chatId: number | null) => {
    dispatch({ type: "SET_HOVERED_CHAT", payload: chatId });
  }, []);

  const setHoveredProjectId = useCallback((projectId: number | null) => {
  ...
  const setSelectedChatId = useCallback((chatId: number | null) => {
  ...
  const setChatToMoveToNewProject = useCallback((chatId: number | null) => {
